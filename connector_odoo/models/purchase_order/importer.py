    ...
    def _after_import(self, binding, force=False):
        ...
        if self.odoo_record.order_line:
            ...
            for line_id in self.odoo_record.order_line:
                purchase_order_line_model = self.env[
                    "odoo.purchase.order.line"
                ].with_delay()
...
class PurchaseOrderLineImporter(Component):
    ...
    def _after_import(self, binding, force=False):
        ...
        if "failed" in pending.mapped("state"):
            _logger.error(
                "Job's {} failed. State can't be changed of Purchase {}".format(
                    pending.mapped("uuid"), self.odoo_record.order_id.name
                )
            )
        if not pending:
