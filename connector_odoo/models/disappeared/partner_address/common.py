...
class OdooResPartnerAddressReferences(models.Model):
    ...
    def default_address_contact_values(self, address, main_partner=False):
        ...
        contact_type = address.type if address.type != "default" else "contact"
        if (
            contact_type == "contact"
            and state_country["state_id"] != main_partner.state_id.id
        ):
    ...
    def import_address(
        self, backend_record, address_id, partner_external_id, force=False
    ):
