...
class StockPickingImporter(Component):
    ...
    def _get_binding_odoo_id_changed(self, binding):
        ...
        if (record.sale_id or record.purchase_id) and record.move_lines:
            ...
            if (
                existing_picking_id
                and not existing_picking_id.bind_ids
...
class OdooPickingMapper(Component):
    ...
    def get_picking_type_from_external_locations(
        ...
        location_importer = self.component(
            usage="record.importer", model_name="odoo.stock.location"
        )
        internal_location_id = binder.to_internal(location_id.id, unwrap=True)
        if not internal_location_id:
            location_importer.run(location_id.id, "odoo.stock.location")
            internal_location_id = binder.to_internal(location_id.id, unwrap=True)
        warehouse_id = internal_location_id.warehouse_id
        if not warehouse_id:
            internal_location_id = binder.to_internal(location_dest_id.id, unwrap=True)
            if not internal_location_id:
                location_importer.run(location_dest_id.id, "odoo.stock.location")
                internal_location_id = binder.to_internal(
                    location_dest_id.id, unwrap=True
                )
            warehouse_id = internal_location_id.warehouse_id
        ...
        result_picking_type_mapping_id = picking_type_mapping_id
        if len(result_picking_type_mapping_id) != 1:
            result_picking_type_mapping_id = picking_type_mapping_id.filtered(
        ...
        return result_picking_type_mapping_id.picking_type_id
